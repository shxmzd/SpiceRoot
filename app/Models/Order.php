...
class Order extends Model
{
    ...
    // Check if order can be rated/reviewed
    public function canBeRated()
    {
        return $this->status === 'delivered';
    }

    // Get order progress steps for buyer tracking
    public function getOrderSteps()
    {
        $baseSteps = [
            ...
            [
                ...
                'date' => $this->status === 'confirmed' ? $this->updated_at->format('M d, Y \a\t h:i A') : null,
                'description' => 'Order confirmed and being prepared'
            ],
            [
                'status' => 'Shipped',
                'completed' => in_array($this->status, ['shipped', 'delivered']),
                'date' => $this->status === 'shipped' ? $this->updated_at->format('M d, Y \a\t h:i A') : null,
                'description' => 'Order has been shipped'
            ],
            [
                'status' => 'Delivered',
                'completed' => $this->status === 'delivered',
                'date' => $this->status === 'delivered' ? $this->updated_at->format('M d, Y \a\t h:i A') : null,
        ...
        if ($this->status === 'cancelled') {
            $baseSteps = [
                ...
                [
                    ...
                    'completed' => true,
                    'date' => $this->updated_at->format('M d, Y \a\t h:i A'),
    ...
    // Get order total amount (alias for consistency)
